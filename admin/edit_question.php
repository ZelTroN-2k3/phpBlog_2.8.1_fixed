    ...
    if (empty($question)) {
        ...
        if (mysqli_stmt_execute($stmt_update_q)) {
            ...
            if ($all_options_inserted) {
                ...
                $message = '<div class="alert alert-danger">Erreur lors de la mise à jour des options.</div>';
            }

        } else {
            $message = '<div class="alert alert-danger">Erreur lors de la mise à jour de la question.</div>';
        }
    }
}
...
if (!$question_data) {
    include 'header.php';
    echo '<section class="content"><div class="alert alert-danger">Question non trouvée ou n\'appartient pas à ce quiz.</div></section>';
    include 'footer.php';
    exit;
}
            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Édition: <?php echo htmlspecialchars($question_data['question']); ?></h3>
                    </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="active" name="active">
                                    <option value="Yes" <?php if ($question_data['active'] == 'Yes') echo 'selected'; ?>>Publié (Active)</option>
                                    <option value="No" <?php if ($question_data['active'] == 'No') echo 'selected'; ?>>Brouillon (Inactive)</option>
                                </select>
                            </div>
                        </div>
            ...
            <div class="input-group mb-2" id="option-group-${optionIndex}">
                <div class="input-group-prepend">
                    <span class="input-group-text">
                        <input type="radio" name="is_correct" value="${optionIndex}" required title="Marquer comme bonne réponse" ${isChecked}>
                    </span>
                </div>
                <input type="text" name="options[${optionIndex}]" class="form-control" placeholder="Texte de la réponse..." value="${escapeHTML(title)}" required>
