...
if (isset($_POST['submit'])) {
    ...
    $old_image_path = ""; // On stocke l'ancien chemin temporairement
    ...
    if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != "") {
        ...
        if($check !== false) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                // On note l'ancienne image pour suppression PLUS TARD, seulement si l'update réussit
    ...
    // EXECUTION AVEC GESTION D'ERREUR
    if (mysqli_stmt_execute($stmt_update)) {
        // Si la mise à jour BDD a réussi, on supprime l'ancienne image physique
        if ($old_image_path != "") {
        ...
        echo '<div class="alert alert-danger m-3">Error updating database: ' . mysqli_stmt_error($stmt_update) . '</div>';
        mysqli_stmt_close($stmt_update);
        // On ne redirige pas pour que vous puissiez lire l'erreur
    }
}
?>
